...
Leave the text out and the box is labelled with its own name (`box parser` draws a box reading "parser"). Write `""` for a deliberately blank box. Inside text, ` / ` — a slash with a space on each side — is a line break; a slash without spaces is an ordinary character, so `TCP/IP` and URLs survive.
...
Node attributes: `style`, `fill`, `stroke`, `subtext`, `size` (`small | normal | large`), `icon`, `shape`, `width` (fold the label every n characters), `gap`, `overlap: allow`, and `align: widths` on a container.
...
- **A lone directional placement binds both axes.** `right of docker` also centres the node vertically on Docker, because walking right from something keeps you on its centre line. That half drops away as soon as another placement claims that axis.
...
`from:` and `to:` name a side — `top`, `bottom`, `left`, `right` — and turn the line into a curve that actually leaves and arrives that way. Name them whenever the straight centre-to-centre line would cut through something.
...
A labelled link widens the corridor between its own two ends by what the label needs, so labels are safe to add.
...
Colours are written directly — any hex or CSS colour, or `none`. There is no list of colour words the tool knows.
...
- **`#` is not a comment.** It opens a hex colour. Comments are `//`.
